...
protocol CURLRequestBodyGenerator {
...
class CURLRequest {
    ...
    struct POSTField {
        ...
        init(name: String, value: String, mimeType: String? = nil) {
            self.name = name
            self.value = value
            self.type = .value
        ...
        init(name: String, filePath: String, mimeType: String? = nil) {
            self.name = name
            self.value = filePath
            self.type = .file
            self.mimeType = mimeType
    ...
    enum Option {
        ...
        /// Path to the key file used for SSH connections.
        sshPublicKey(String),
    ...
    convenience init(_ url: String, options: [Option] = []) {
        self.init(options: [.url(url)] + options)
    }
    /// Init with url and one or more options.
    convenience init(_ url: String, _ option1: Option, _ options: Option...) {
